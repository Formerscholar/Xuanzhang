...
export default {
  setup(props, { root }) {
    ...
    async function getEditReceiving() {
      const { data } = await getEditReceivingInformation(
        getEditReceivingInformationData.value
      )
      const { consignee, consignee_tel, consignee_address } = data.receiving
      state.name = consignee
      state.tel = consignee_tel
      state.addressDetail = consignee_address
    }
    ...
    async function onSave() {
      const { code, msg } = await editReceivingInformation(
