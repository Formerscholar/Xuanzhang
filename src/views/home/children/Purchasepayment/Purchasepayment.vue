...
export default {
  ...
  setup(props, { root }) {
    ...
    function printClick(iid) {
      state.isShow = true
      state.textContent = `http://219.83.161.11:8030/view/html/accountment/sell_print.php?id=${iid}&order_type=warehousingM`
    }
    ...
    async function getAddSettlement() {
      ...
      if (data.companyOrderType['warehousingM'] === undefined) {
        root.$router.replace('/home')
