        ...
        <van-swipe-cell v-if="item.to_examine != undefined ">
          <div class="box-card">
            <div :class="item.deleted_at && 'color_break ' ">
              ...
              <span :class="item.deleted_at ? 'color_break name' :' name'">{{item.distributor_name}}</span>
              <div class="time_box">
                <span
...
export default {
  ...
  setup(props, { root }) {
    ...
    function printClick(iid) {
      state.isShow = true
      state.textContent = `http://219.83.161.11:8030/view/html/accountment/sell_print.php?id=${iid}&order_type=settlement`
    }
    ...
    async function getAddSettlement() {
      ...
      if (data.companyOrderType['settlement'] === undefined) {
        root.$router.replace('/home')
